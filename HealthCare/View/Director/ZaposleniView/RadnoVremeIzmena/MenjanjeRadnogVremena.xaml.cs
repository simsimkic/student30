...
namespace HCIProjekat.ZaposleniView
{
    ...
    /// </summary>
    public partial class MenjanjeRadnogVremena : Page
    {
        ...
        private const string INVALID_DAYS_SELECTED = "Izabrani zaposleni ima zakazane termine u izabranim datumima!";
        ...
        public MenjanjeRadnogVremena(DateTime dateFrom, DateTime dateTo, UserControl Employee)
        {
            ...
            workTimes = _workTimeController.GetWorkTime(user).ToList();
        ...
        private void confirmaAddingWorkTimeEvent(object sender, RoutedEventArgs e)
        {
            ...
            {
                List<WorkTime> mutualWorkTimes = getMutual();
                if (_workTimeController.UpdateWhileOverriding(mutualWorkTimes, createWorkTime()) == null)
        ...
        private List<WorkTime> getMutual()
        {
            List<WorkTime> retVal = new List<WorkTime>();
            foreach (var v in workTimes)
            {
                if((_dateFrom <= v.StartDate.Date && _dateTo >= v.StartDate.Date) || (_dateFrom.Date >= v.StartDate.Date && _dateFrom.Date <= v.EndDate.Date))
                    retVal.Add(v);
            }
        ...
        private WorkTime createWorkTime()
        {
            ...
            if (sviDani.IsChecked == true)
            {
                ...
                /*                workDays.Add(new WorkDay() { Day = Days.Everydays, StartTime = int.Parse(sdOd.Text), EndTime = int.Parse(sdDo.Text) });
                */
            }
            else
            {
